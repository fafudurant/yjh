<template>
    <div class="app-container" style="padding: 10px 10px;">
        <h4>基本信息</h4>
        <table class="table table-striped table-condenseda table-bordered active_content" width="100%">
            <tbody>
                <tr>
                    <th width="15%">医院名称</th>
                    <td width="35%"><b style="font-size: 14px">{{ hospital.hosname }}</b> | <span>{{ hospital.param.hostypeString }}</span></td>
    ...
                    <th>退号时间</th>
                    <td><span>{{ bookingRule.quitDay == -1 ? '就诊前一工作日' : '就诊当日' }}</span><span>{{ bookingRule.quitTime }}前取消</span></td>
                </tr>
                <tr>
                    <th>预约规则</th>
                    <td colspan="3">
